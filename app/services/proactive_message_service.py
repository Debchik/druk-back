        ...
        try:
            ...
            if proactive_message.reason == 'reminder':
                ...
                system_prompt = (
                    character_context
                    + '\n\n'
                    'Это не запрос на создание напоминания и не просьба подтвердить его постановку. '
                    'Время уже наступило, поэтому сейчас нужно отправить пользователю само напоминание. '
                    'Сформулируй готовое сообщение, которое мягко побуждает сделать действие прямо сейчас. '
                    'Обязательно назови действие из напоминания.\n'
                    'Запрещено: говорить «я запомню», «я напомню позже», «обязательно напомню», '
                    '«я поставил напоминание», сообщать о технических деталях или описывать прошлое обещание. '
                    'Не утверждай, что пользователь уже выполнил действие. '
                    'Используй настоящее время и формулировки вроде «пора», «самое время», «напоминаю:». '
                    'Ответ должен быть на языке пользователя и содержать максимум 2 предложения.\n'
                    f'Изначальная просьба пользователя: {original_text}\n'
                    f'Действие, о котором нужно напомнить прямо сейчас: {proactive_message.content}'
                )
                prompt_messages.append({
                    'role': 'user',
                    'content': (
                        'Сейчас наступило время напоминания. Напиши одно готовое сообщение пользователю '
                        'с напоминанием сделать указанное действие прямо сейчас. Не подтверждай постановку '
                        'и не обещай напомнить в будущем.'
                    ),
                })
            else:
                system_prompt = (
